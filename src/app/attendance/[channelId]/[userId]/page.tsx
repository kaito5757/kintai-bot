...
export default function UserAttendancePage() {
  ...
  const [user, setUser] = useState<WorkSession['user'] | null>(null);
  ...
  const fetchAttendanceData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/admin/attendance?userId=${encodeURIComponent(userId)}&channelId=${encodeURIComponent(channelId)}`);
      ...
      if (result.success) {
        ...
        if (result.data.length > 0) {
          ...
          setChannelName(result.data[0].channel?.channelName || channelId);
        }
      }
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            @{user?.username} | {channelName || channelId}
          </p>
        </div>
                        ...
                        <div className="px-6 py-4 border-t border-gray-100">
                          ...
                          <div className="space-y-2">
                            {daySessions.flatMap(session => session.breaks).length === 0 ? (
                              ...
                              daySessions.flatMap(session => session.breaks).map((breakItem) => (
